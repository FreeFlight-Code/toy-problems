/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*
  80s KIDS #6: ROCK 'EM SOCK 'EM ROBOTS
  https://www.codewars.com/kata/80-s-kids-number-6-rock-em-sock-em-robots/train/javascript
...
function fight(robot1, robot2, tactics) {
 var attacker = robot2.speed > robot1.speed ? robot2 : robot1;
 var defender = robot2.speed > robot1.speed ? robot1 : robot2;

 function noOneIsDead() {
   return robot1.health > 0 && robot2.health > 0;
 }
 function someTacticsRemain() {
   return robot1.tactics.length > 0 || robot2.tactics.length > 0;
 }
 function attack() {
   if (attacker.tactics.length > 0)  {
     defender.health -= tactics[attacker.tactics.splice(0, 1)];
   }
 }
 function switchAttacker() {
   attacker = attacker === robot1 ? robot2 : robot1;
   defender = defender === robot1 ? robot2 : robot1;
 }
 function outcome() {
 ...
 while (noOneIsDead() && someTacticsRemain()) {
   attack();
   switchAttacker();
 }
 return outcome();
