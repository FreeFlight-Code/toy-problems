...
function splitWithoutLoss(s, d) {
  var noPipe = d.replace('|', '');
  var allInstances = new RegExp(noPipe, 'g');
  s = s.replace(allInstances, d);

  return s.split('|').filter(function(c){return c});
}
