...
var decodeMorse = function(morseCode) {
	...
	// var answer = '';
	var newArr = [];
	morseCode.split('  ').forEach(function(element) {
		var word = '';
		element.split(' ').forEach(function(e, i, arr) {
			arr[i] = (key[e])
			// element.join('');
			word = arr.filter(function(letter) {
				if (letter !== ' ') {
					return true;
				}
			});
	...
	newArr = newArr.join(' ').replace(/\s\s/, '').replace(/^\s/, '').replace(/\s$/, '');
	return newArr;
};
