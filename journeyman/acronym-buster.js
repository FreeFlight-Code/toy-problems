/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*
  ACRONYM BUSTER
  https://www.codewars.com/kata/acronym-buster/train/javascript
...
function acronymBuster(string){
  string = string.replace(/KPI/g, 'key performance indicators').replace(/EOP/g, 'the end of the day').replace(/TBD/g, 'to be decided').replace(/WAH/g, 'work at home').replace(/IAM/g, 'in a meeting').replace(/OOO/g, 'out of office').replace(/NRN/g, 'no reply necessary').replace(/CTA/g, 'call to action').replace(/SWOT/g, 'strengths, weaknesses, opportunities and threats').replace(/^[a-z]/, function(startOfSentence) {return startOfSentence.toUpperCase();}).replace(/\.\s[a-z]/g, function(v) {return v.toUpperCase();});

  var acronym = string.match(/[A-Z]{3,}/);

  if (acronym) return acronym[0] + ' is an acronym. I do not like acronyms. Please remove them from your email.';
  else return string;
}
